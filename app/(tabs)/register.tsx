            ...
            <ScrollView contentContainerStyle={{ paddingTop: headerHeight, paddingBottom: bottomBarHeight }} contentContainerClassName='w-full items-center'>
                <View className='bg-white items-center rounded-[30px] w-[80%] p-10 dark:bg-[#171a1c] shadow-md'>
                    <View style={{ borderBottomColor: name.length === 0 ? 'gray' : nValid.valid ? 'gray': '#f2545b' }} className='flex-row items-center border-b-2 border-b-[gray]'>
                        ...
                        <StyledTextInput placeholderTextColor="gray" placeholder={t('input.username')} className='text-black dark:text-white ml-1 w-[80%] text-3xl' onChangeText={setName}/>
                    </View>
                    { !nValid.valid && name.length !== 0 && nValid.messages.map((msg, i) => (
                        <StyledText className='text-[#f2545b] mt-2' key={i}>{msg}</StyledText>
                    ))}
                    ...
                    <View style={{ borderBottomColor: pass.length === 0 ? 'gray' : pValid.valid ? 'gray': '#f2545b' }} className='flex-row items-center border-b-2 border-b-[gray] mt-5'>
                        ...
                        <StyledTextInput placeholderTextColor="gray" placeholder={t('input.pass')} className='text-black dark:text-white ml-1 w-[80%] text-3xl' onChangeText={setPass} secureTextEntry={hide} autoCapitalize="none" autoCorrect={false} />
                    </View>
                    ...
                    <View style={{ borderBottomColor: repass.length === 0 ? 'gray' : repass === pass ? 'gray': '#f2545b' }}  className='flex-row items-center border-b-2 border-b-[gray] mt-5'>
                        ...
                        <StyledTextInput placeholderTextColor="gray" placeholder={t('input.rePass')} className='text-black dark:text-white ml-1 w-[80%] text-3xl' onChangeText={setRepass} secureTextEntry={reHide} autoCapitalize="none" autoCorrect={false} multiline={false} numberOfLines={1}/>
                    </View>
                    <Pressable onPress={() => setReHide(v => !v)} className='items-end px-3 w-full'><StyledText style={{color: 'gray'}}>{ reHide ? t('input.option.show') : t('input.option.hide') }</StyledText></Pressable>
                    ...
                    <View className='flex-column items-center mt-2 gap-1'>
                        <Pressable onPress={() => router.push('/login')} className='group'>
                            <StyledText className='text-base text-[gray] dark:group-active:text-[lightgray] group-active:text-black'>{t('auth.register.haveAccount')}</StyledText>
                        </Pressable>
                    </View>
                </View>
                <Pressable onPress={() => fRegister(name, email, pass, openModal, t)} disabled={ !(pValid.valid && pass === repass && eValid && nValid.valid) } className='shadow-md dark:bg-[#1e3773] bg-[#4974d7] w-[80%] p-5 mt-5 rounded-[50px] active:opacity-80'>
                    <StyledText className='text-2xl self-center text-white'>{t('input.button.register')}</StyledText>
                </Pressable>
            </ScrollView>
        </View>
    );
